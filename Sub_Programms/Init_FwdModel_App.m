...
app.Path.Programms = [ app.Path.CurrentFolder '\Sub_Programms'];
app.Path.Setups = [app.Path.CurrentFolder '\Setups'];
...
app.Path.ChambersDesigns=[app.Path.CurrentFolder '\Setups\ChambersDesigns'];
app.Path.fmdl=[app.Path.CurrentFolder '\fmdl'];
...
if Reset
    ...
    clear global EIDORS
    global EIDORS
    tmp =load([app.Path.Setups '\EIDORS_default.mat']);
    EIDORS=tmp.EIDORS;
